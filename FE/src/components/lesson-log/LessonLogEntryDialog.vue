...
const props = withDefaults(defineProps<{ visible: boolean; entry: LessonLogEntry | null; readOnly?: boolean; saving?: boolean; errorMessage?: string }>(), { readOnly: false, saving: false, errorMessage: '' })
const emit = defineEmits<{ 'update:visible': [boolean]; saveDraft: [LessonLogRequestFields]; submit: [LessonLogRequestFields] }>()
...
const submitValid = computed(() => draftValid.value && Boolean(form.value.title?.trim()) && form.value.completionStatus !== null && form.value.grade !== null && form.value.presentCount !== null && form.value.presentCount !== undefined && form.value.absentCount !== null && form.value.absentCount !== undefined && Number(form.value.presentCount) >= 0 && Number(form.value.absentCount) >= 0 && Number(form.value.presentCount) + Number(form.value.absentCount) === props.entry?.rosterCountSnapshot)
function saveDraft(): void { emit('saveDraft', { ...form.value }) }
function submit(): void { if (submitValid.value) emit('submit', { ...form.value }) }
</script>
<template>
  <Dialog :visible="props.visible" modal :header="props.entry?.entryId ? 'Cập nhật sổ đầu bài' : 'Ghi sổ đầu bài'" :style="{ width: 'min(720px, 96vw)' }" @update:visible="emit('update:visible', $event)">
