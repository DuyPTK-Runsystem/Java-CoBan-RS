  ...
  <Dialog :visible="props.visible" modal :header="heading" :style="{ width: 'min(100% - 2rem, 720px)' }" :closable="!props.saving" @update:visible="emit('update:visible', $event)">
    <FormAlert v-if="props.errorMessage" tone="error" :message="props.errorMessage" /><form class="form-stack" novalidate @submit.prevent="save">
      <div class="catalog-form-grid">
        <div class="field-group"><label for="teacher-code">Mã giáo viên <span class="required-mark" aria-hidden="true">*</span></label><InputText id="teacher-code" v-model="values.teacherCode" maxlength="50" :invalid="Boolean(errors.teacherCode)" aria-required="true" fluid /><small v-if="errors.teacherCode" class="field-error">{{ errors.teacherCode }}</small></div>
        <div class="field-group"><label for="teacher-name">Họ và tên <span class="required-mark" aria-hidden="true">*</span></label><InputText id="teacher-name" v-model="values.teacherName" maxlength="150" :invalid="Boolean(errors.teacherName)" aria-required="true" fluid /><small v-if="errors.teacherName" class="field-error">{{ errors.teacherName }}</small></div>
        ...
        <div class="field-group"><label for="teacher-status">Trạng thái <span class="required-mark" aria-hidden="true">*</span></label><Select id="teacher-status" v-model="values.status" :options="statusOptions" option-label="label" option-value="value" :invalid="Boolean(errors.status)" aria-required="true" fluid /><small v-if="errors.status" class="field-error">{{ errors.status }}</small></div>
