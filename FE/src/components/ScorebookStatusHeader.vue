...
<template>
  <section class="content-surface">
    ...
    <div v-if="props.scorebook" class="page-heading-actions"><Button label="Tải lại" icon="pi pi-refresh" outlined :loading="props.loading" @click="$emit('reload')" /><Button label="Mở sổ" icon="pi pi-lock-open" :disabled="props.scorebook.status !== 'DRAFT'" :loading="props.loading" @click="$emit('open')" /><Button label="Công bố" icon="pi pi-check" severity="success" :disabled="props.scorebook.status !== 'OPEN'" :loading="props.loading" @click="$emit('publish')" /></div>
    <p v-else class="field-hint">Chọn môn học để tìm sổ điểm hiện có hoặc tạo mới khi được cấp quyền.</p>
    <p v-if="props.scorebook?.status === 'PUBLISHED' || props.scorebook?.status === 'CLOSED'" class="field-hint">Sổ điểm đang ở chế độ chỉ đọc.</p>
  </section>
</template>
