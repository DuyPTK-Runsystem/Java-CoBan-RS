...
<template>
  <Dialog :visible="props.visible" modal header="Lịch sử xếp lớp học sinh" :style="{ width: 'min(100% - 2rem, 860px)' }" @update:visible="emit('update:visible', $event)">
    <div class="status-dialog-content">
      <div class="status-dialog-heading"><div><h2>{{ props.studentCode }} · {{ props.studentName }}</h2></div><i class="pi pi-history" aria-hidden="true" /></div>
      ...
      <div v-else-if="pageState() === 'empty'" class="empty-state"><i class="pi pi-history" aria-hidden="true" /><strong>Chưa có lịch sử xếp lớp</strong><p>Học sinh chưa có bản ghi xếp lớp nào.</p></div>
      <div v-else class="history-list">
        <article v-for="item in props.history" :key="item.enrollment.id" class="history-card">
          ...
          <dl class="detail-grid"><div class="detail-item"><span>Mã học sinh</span><strong>{{ item.enrollment.studentCode }}</strong></div><div class="detail-item"><span>Hoàn thành</span><strong>{{ formatDate(item.enrollment.completedAt) }}</strong></div></dl>
