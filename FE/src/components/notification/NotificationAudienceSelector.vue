...
<template>
  <div class="notification-audience-selector" data-testid="audience-selector">
    ...
    <div v-if="currentAudienceType === 'CLASS'" class="field-group">
      <label for="notification-class-picker">Lớp học nhận thông báo</label>
      <Dropdown id="notification-class-picker" :model-value="selectedClassId" :options="classOptions" option-value="classId" placeholder="Chọn lớp học" :loading="classLoading" :disabled="props.disabled" filter filter-placeholder="Tìm theo mã hoặc tên lớp" show-clear fluid @filter="handleClassFilter" @update:model-value="(value) => selectClass(classOptions.find((item) => item.classId === value) ?? null)">
...
<style scoped>
.notification-audience-selector { display: grid; gap: 18px; }.notification-audience-selector :deep(.p-dropdown), .notification-audience-selector :deep(.p-inputtext) { width: 100%; }.notification-audience-selector label { color: #334155; font-size: 14px; font-weight: 600; }.notification-audience-selector small { display: block; }.field-hint { color: #64748b; }.field-error { color: #b91c1c; }.inline-retry { padding: 0; border: 0; background: transparent; color: #b91c1c; text-decoration: underline; cursor: pointer; }.audience-option { display: grid; gap: 3px; }.audience-option small, .result-copy small { color: #64748b; }.field-group-heading, .lookup-footer { display: flex; align-items: center; justify-content: space-between; gap: 12px; }.field-group-heading > div { display: grid; gap: 4px; }.audience-filter-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px; }.audience-filter-grid .field-group { gap: 6px; }.audience-results { display: grid; max-height: 300px; overflow-y: auto; border: 1px solid #cbd5e1; border-radius: 6px; }.lookup-state { padding: 14px; color: #64748b; }.audience-result { display: flex; align-items: flex-start; gap: 10px; padding: 10px 12px; border: 0; border-bottom: 1px solid #e2e8f0; background: #fff; color: #1e293b; text-align: left; cursor: pointer; }.audience-result:last-child { border-bottom: 0; }.audience-result:hover, .audience-result-selected { background: #eff6ff; }.audience-result:disabled { cursor: not-allowed; opacity: .6; }.result-copy { display: grid; gap: 3px; }.selected-user-chips { display: flex; flex-wrap: wrap; gap: 8px; }.user-chip { display: inline-flex; align-items: center; gap: 6px; max-width: 100%; padding: 6px 9px; border-radius: 999px; background: #dbeafe; color: #1e3a8a; font-size: 13px; }.user-chip button { border: 0; background: transparent; color: inherit; cursor: pointer; font-size: 16px; line-height: 1; }.user-chip button:disabled { cursor: not-allowed; }.pagination-actions { display: flex; gap: 4px; }
