...
describe('PlacementWorkspaceReview', () => {
  ...
  it('renders class metadata and a result needing manual handling without blocking confirm', () => {
    ...
    expect(wrapper.text()).toContain('Sĩ số hiện tại: 28')
    expect(wrapper.text()).toContain('Sĩ số hiện tại: 24')
    expect(wrapper.text()).not.toContain('Cách phân lớp, sĩ số và mục tiêu nam nữ do hệ thống cung cấp.')
