...
<template>
  <Dialog :visible="props.visible" modal header="Gán môn cho lớp" :style="{ width: 'min(100% - 2rem, 600px)' }" :closable="!props.saving" @update:visible="emit('update:visible', $event)">
    ...
    <form class="form-stack" novalidate @submit.prevent="save">
      <div class="field-group"><label for="class-subject-subject">Môn học đang giảng dạy đã được áp dụng <span v-if="props.mode === 'create'" class="required-mark" aria-hidden="true">*</span></label><Select id="class-subject-subject" v-model="values.subjectId" :options="subjectOptions" option-label="displayName" option-value="id" placeholder="Chọn môn học" :disabled="isReadOnly || props.mode === 'edit'" :invalid="Boolean(errors.subjectId)" fluid :aria-required="props.mode === 'create'" /><small v-if="errors.subjectId" class="field-error">{{ errors.subjectId }}</small></div>
