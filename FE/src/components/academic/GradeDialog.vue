  ...
  <Dialog :visible="props.visible" modal :header="heading" :style="{ width: 'min(100% - 2rem, 640px)' }" :closable="!props.saving" @update:visible="emit('update:visible', $event)">
    ...
    <form class="form-stack" novalidate @submit.prevent="save">
      <div class="catalog-form-grid">
        <div class="field-group"><label for="grade-code">Mã khối <span class="required-mark" aria-hidden="true">*</span></label><InputText id="grade-code" v-model="values.code" maxlength="10" placeholder="Ví dụ: GRADE_6" :invalid="Boolean(errors.code)" fluid aria-required="true" /><small v-if="errors.code" class="field-error">{{ errors.code }}</small></div>
        <div class="field-group"><label for="grade-name">Tên khối <span class="required-mark" aria-hidden="true">*</span></label><InputText id="grade-name" v-model="values.name" maxlength="50" placeholder="Ví dụ: Khối 6" :invalid="Boolean(errors.name)" fluid aria-required="true" /><small v-if="errors.name" class="field-error">{{ errors.name }}</small></div>
        <div class="field-group"><label for="grade-level">Cấp <span class="required-mark" aria-hidden="true">*</span></label><Select id="grade-level" v-model="values.gradeLevel" :options="gradeOptions" option-label="label" option-value="value" placeholder="Chọn cấp" :invalid="Boolean(errors.gradeLevel)" fluid aria-required="true" /><small v-if="errors.gradeLevel" class="field-error">{{ errors.gradeLevel }}</small></div>
