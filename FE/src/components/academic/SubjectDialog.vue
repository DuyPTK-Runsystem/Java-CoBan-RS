  ...
  <Dialog :visible="props.visible" modal :header="props.mode === 'edit' ? 'Chỉnh sửa môn học' : 'Tạo môn học'" :style="{ width: 'min(100% - 2rem, 600px)' }" :closable="!props.saving" @update:visible="emit('update:visible', $event)">
    ...
    <form class="form-stack" novalidate @submit.prevent="save">
      <div class="catalog-form-grid">
        <div class="field-group"><label for="subject-code">Mã môn <span class="required-mark" aria-hidden="true">*</span></label><InputText id="subject-code" v-model="values.code" maxlength="30" placeholder="Ví dụ: MAT" :invalid="Boolean(errors.code)" fluid aria-required="true" /><small v-if="errors.code" class="field-error">{{ errors.code }}</small></div>
        <div class="field-group"><label for="subject-name">Tên môn <span class="required-mark" aria-hidden="true">*</span></label><InputText id="subject-name" v-model="values.name" maxlength="150" placeholder="Ví dụ: Toán" :invalid="Boolean(errors.name)" fluid aria-required="true" /><small v-if="errors.name" class="field-error">{{ errors.name }}</small></div>
