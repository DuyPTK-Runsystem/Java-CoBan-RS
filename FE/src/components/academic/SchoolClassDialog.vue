  ...
  <Dialog :visible="props.visible" modal :header="heading" :style="{ width: 'min(100% - 2rem, 640px)' }" :closable="!props.saving" @update:visible="emit('update:visible', $event)">
    ...
    <form class="form-stack" novalidate @submit.prevent="save">
      <div class="catalog-form-grid">
        <div class="field-group"><label for="class-year">Năm học <span class="required-mark" aria-hidden="true">*</span></label><Select id="class-year" v-model="values.academicYearId" :options="props.academicYears" option-label="code" option-value="id" placeholder="Chọn năm học" :disabled="isClosed || isEdit" :invalid="Boolean(errors.academicYearId)" fluid aria-required="true" /><small v-if="errors.academicYearId" class="field-error">{{ errors.academicYearId }}</small></div>
        <div class="field-group"><label for="class-grade">Khối <span class="required-mark" aria-hidden="true">*</span></label><Select id="class-grade" v-model="values.gradeLevelId" :options="props.grades" option-label="name" option-value="id" placeholder="Chọn khối" :disabled="isClosed" :invalid="Boolean(errors.gradeLevelId)" fluid aria-required="true" /><small v-if="errors.gradeLevelId" class="field-error">{{ errors.gradeLevelId }}</small></div>
        <div class="field-group"><label for="class-code">Mã lớp <span class="required-mark" aria-hidden="true">*</span></label><InputText id="class-code" v-model="values.classCode" maxlength="30" placeholder="Ví dụ: 6A1" :disabled="isClosed" :invalid="Boolean(errors.classCode)" fluid aria-required="true" /><small v-if="errors.classCode" class="field-error">{{ errors.classCode }}</small></div>
