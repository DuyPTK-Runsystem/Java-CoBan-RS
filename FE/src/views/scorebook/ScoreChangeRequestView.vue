...
import ScoreChangeRequestDetail from '@/components/score-change/ScoreChangeRequestDetail.vue'
import ScoreChangeRequestForm, { type ScoreChangeRequestFormContext } from '@/components/score-change/ScoreChangeRequestForm.vue'
...
<template>
  <main class="content-page score-change-page">
    <header class="page-heading"><div><h1>Yêu cầu sửa điểm</h1></div><Button v-if="canUseModule && scorebook" label="Tạo yêu cầu" icon="pi pi-plus" :disabled="scoreGridLoading || !scoreGrid" @click="openCreateForm" /></header>
...
.table-scroll { overflow-x: auto; } table { width: 100%; border-collapse: collapse; min-width: 760px; } th, td { padding: 12px 10px; border-bottom: 1px solid var(--surface-border); text-align: left; } th { color: var(--text-color-secondary); font-size: .85rem; vertical-align: bottom; } tbody td { vertical-align: middle; } .student-cell { min-width: 155px; } .student-code, .student-name { display: block; } .student-name { color: var(--text-color-secondary); font-size: .875rem; line-height: 1.35; }
