...
import ScoreChangeRequestForm from '@/components/ScoreChangeRequestForm.vue'
...
import { fetchScorebookByClassSubject } from '@/services/scorebookApi'
import { getAuthSession } from '@/services/authSession'
import { isApiError } from '@/types/api'
import type { AcademicYear, ClassSubject, SchoolClass, Semester, Subject } from '@/types/academic'
import type { Scorebook } from '@/types/scorebook'
...
async function loadScorebook(): Promise<void> { scorebook.value = classSubjectId.value === null ? null : await fetchScorebookByClassSubject(token.value, classSubjectId.value) }
...
<template>
  <main class="content-page score-change-page">
    <header class="page-heading"><div><p class="eyebrow">V2 · SCORE CHANGE</p><h1>Yêu cầu sửa điểm</h1><p>Gửi và theo dõi yêu cầu sửa điểm theo bối cảnh lớp học, không cần nhớ mã kỹ thuật.</p></div><Button v-if="canUseModule && scorebook" label="Tạo yêu cầu" icon="pi pi-plus" @click="formVisible = true" /></header>
...
.filter-bar { grid-template-columns: 220px minmax(220px, 1fr) auto; margin-bottom: 18px; }
...
@media (max-width: 560px) { .context-grid, .filter-bar, .detail-grid { grid-template-columns: 1fr; } }
</style>
