...
async function remove(student: Student): Promise<void> { const accessToken = token(); if (!accessToken) return; try { await deleteStudent(accessToken, student.studentId); if (students.value.length === 1 && query.value.page > 0) query.value.page -= 1; await load(); statusMessage.value = 'Student deleted.' } catch (error) { if (isApiError(error, 401)) return; errorMessage.value = error instanceof Error ? error.message : 'Unable to delete student.' } }
...
function logout(): void { const session = getAuthSession(); if (!session) { clearAuthSession(); void router.replace('/login'); return }; void logoutApi(session.accessToken).catch(() => undefined).finally(() => { clearAuthSession(); if (router.currentRoute.value.name !== 'login') return router.replace('/login') }) }
onMounted(() => { void load() })
</script>
