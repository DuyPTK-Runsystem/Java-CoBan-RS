...
import SchoolClassDialog from '@/components/academic/SchoolClassDialog.vue'
import SchoolClassTable from '@/components/academic/SchoolClassTable.vue'
...
async function loadClasses(): Promise<void> {
  ...
  try {
    ...
    errorMessage.value = extractApiErrorMessage(error, 'Không thể tải danh sách lớp.')
...
async function save(values: SchoolClassFormValues): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    dialogErrorMessage.value = extractApiErrorMessage(error, 'Không thể lưu lớp.')
  } finally {
    saving.value = false
  }
}
...
async function close(schoolClass: SchoolClass): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    errorMessage.value = extractApiErrorMessage(error, 'Không thể đóng lớp.')
  }
}
...
async function remove(schoolClass: SchoolClass): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    errorMessage.value = extractApiErrorMessage(error, 'Không thể xóa lớp.')
  }
}
