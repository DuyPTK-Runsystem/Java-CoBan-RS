...
import ClassSubjectDialog from '@/components/academic/ClassSubjectDialog.vue'
import ClassSubjectTable from '@/components/academic/ClassSubjectTable.vue'
...
async function loadClassSubjects(): Promise<void> {
  ...
  try {
    ...
    errorMessage.value = extractApiErrorMessage(error, 'Không thể tải danh sách lớp-môn.')
...
async function loadYearContext(): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    forbidden.value = isApiError(error, 403)
    errorMessage.value = extractApiErrorMessage(error, 'Không thể tải lớp và học kỳ theo năm học.')
    loadingState.value = 'error'
  } finally {
...
async function loadContext(): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    forbidden.value = isApiError(error, 403)
    errorMessage.value = extractApiErrorMessage(error, 'Không thể tải context năm học và môn học.')
    loadingState.value = 'error'
  } finally {
    contextLoading.value = false
  }
...
async function save(values: ClassSubjectFormValues): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    if (isApiError(error, 409)) conflictMessage.value = extractApiErrorMessage(error, 'Môn chưa được cấu hình applicability hoặc đã tồn tại trong lớp-môn.')
    else dialogErrorMessage.value = extractApiErrorMessage(error, 'Không thể lưu lớp-môn.')
  } finally {
    saving.value = false
  }
}
