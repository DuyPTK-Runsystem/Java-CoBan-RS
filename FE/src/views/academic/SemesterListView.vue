...
import SemesterDialog from '@/components/academic/SemesterDialog.vue'
import SemesterStatusDialog from '@/components/academic/SemesterStatusDialog.vue'
import SemesterTable from '@/components/academic/SemesterTable.vue'
...
const { requireAccessToken: accessToken } = useAuthSession()
...
async function load(): Promise<void> {
  ...
  try {
    ...
    errorMessage.value = extractApiErrorMessage(error, 'Không thể tải danh sách học kỳ.')
    loadingState.value = 'error'
  }
}
...
async function saveSemester(values: SemesterFormValues): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    dialogErrorMessage.value = extractApiErrorMessage(error, 'Không thể lưu học kỳ.')
  } finally {
    saving.value = false
  }
}
...
async function activate(semester: Semester): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    errorMessage.value = extractApiErrorMessage(error, 'Không thể kích hoạt học kỳ.')
  } finally {
...
async function loadReport(semester: Semester): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    if (selectedSemester.value?.id === semester.id) statusError.value = extractApiErrorMessage(error, 'Không thể tải báo cáo hoàn thành dữ liệu.')
  } finally {
...
async function loadNotifications(semester: Semester): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    if (selectedSemester.value?.id === semester.id) notificationError.value = extractApiErrorMessage(error, 'Không thể tải lịch sử gửi email.')
  } finally {
...
async function lock(semester: Semester): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    statusError.value = extractApiErrorMessage(error, 'Không thể khóa học kỳ.')
  } finally {
    actionLoading.value = false
  }
}
...
async function submitReopen(): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    reopenError.value = extractApiErrorMessage(error, 'Không thể mở lại học kỳ.')
  } finally {
    actionLoading.value = false
  }
}
