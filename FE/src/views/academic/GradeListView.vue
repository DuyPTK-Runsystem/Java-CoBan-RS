...
import GradeDialog from '@/components/academic/GradeDialog.vue'
import GradeTable from '@/components/academic/GradeTable.vue'
...
async function load(): Promise<void> {
  ...
  try {
    ...
    errorMessage.value = extractApiErrorMessage(error, 'Không thể tải danh sách khối.')
...
async function save(values: GradeLevelFormValues): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    dialogErrorMessage.value = extractApiErrorMessage(error, 'Không thể lưu khối.')
  } finally {
    saving.value = false
  }
}
...
async function toggleActive(grade: GradeLevel): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    errorMessage.value = extractApiErrorMessage(error, 'Không thể thay đổi trạng thái khối.')
  }
}
...
async function remove(grade: GradeLevel): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    errorMessage.value = extractApiErrorMessage(error, 'Không thể xóa khối.')
  }
}
