...
import SubjectApplicabilityDialog from '@/components/academic/SubjectApplicabilityDialog.vue'
import SubjectDialog from '@/components/academic/SubjectDialog.vue'
import SubjectTable from '@/components/academic/SubjectTable.vue'
...
const { roles, requireAccessToken: token } = useAuthSession()
...
const readOnly = computed(() => roles.value.includes('TEACHER') && !roles.value.some((role) => role === 'ADMIN' || role === 'ACADEMIC_OFFICE'))
...
async function loadSubjects(): Promise<void> {
  ...
  try {
    ...
    errorMessage.value = extractApiErrorMessage(error, 'Không thể tải danh sách môn học.')
...
async function loadYearContext(): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    forbidden.value = isApiError(error, 403)
    errorMessage.value = extractApiErrorMessage(error, 'Không thể tải học kỳ và lớp theo năm học.')
  }
}
...
async function loadContext(): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    forbidden.value = isApiError(error, 403)
    errorMessage.value = extractApiErrorMessage(error, 'Không thể tải context năm học và khối.')
  } finally {
...
async function saveSubject(values: SubjectFormValues): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    dialogErrorMessage.value = extractApiErrorMessage(error, 'Không thể lưu môn học.')
  } finally {
    saving.value = false
  }
}
...
async function loadApplicabilities(): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    applicabilityErrorMessage.value = extractApiErrorMessage(error, 'Không thể tải danh sách phạm vi áp dụng.')
  } finally {
...
async function saveApplicability(values: SubjectApplicabilityFormValues): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    applicabilityErrorMessage.value = extractApiErrorMessage(error, 'Không thể lưu cấu hình phạm vi áp dụng.')
  } finally {
    saving.value = false
  }
}
...
async function deactivateApplicability(applicability: SubjectApplicability): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    applicabilityErrorMessage.value = extractApiErrorMessage(error, 'Không thể ngừng áp dụng cấu hình.')
  } finally {
    saving.value = false
  }
}
...
async function reactivateApplicability(applicability: SubjectApplicability): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    applicabilityErrorMessage.value = extractApiErrorMessage(error, 'Không thể kích hoạt lại cấu hình.')
  } finally {
    saving.value = false
  }
}
