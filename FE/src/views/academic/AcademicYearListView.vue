...
import AcademicYearDialog from '@/components/academic/AcademicYearDialog.vue'
...
async function saveAcademicYear(values: AcademicYearFormValues): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    dialogErrorMessage.value = extractApiErrorMessage(error, 'Không thể lưu năm học.')
  } finally {
    saving.value = false
  }
}
...
async function close(academicYear: AcademicYear): Promise<void> {
  ...
  try {
    ...
    if (isApiError(error, 401)) return
    errorMessage.value = extractApiErrorMessage(error, 'Không thể đóng năm học.')
  }
}
