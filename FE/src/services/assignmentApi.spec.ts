...
describe('assignmentApi', () => {
  ...
  it('maps all assignment endpoints without changing date-only values', async () => {
    ...
    await createSubjectTeachingAssignment('t', 5, request)
    await replaceSubjectTeachingAssignment('t', 6, request)
    await endSubjectTeachingAssignment('t', 6, { validTo: '2027-01-01' })
    ...
    expect(fetchMock.mock.calls.map(([url]) => url)).toEqual([
      ...
      'http://localhost:8081/api/v2/class-subjects/5/teaching-assignments',
  ...
  it('loads teaching assignments for a timetable class and semester', async () => {
    ...
    await fetchSubjectAssignmentsByClass('token', 6, 2)

    expect(fetchMock.mock.calls[0][0])
      .toContain('/api/v2/assignments/classes/6/subjects?semesterId=2')
  })
})
