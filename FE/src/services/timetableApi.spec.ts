...
describe('timetableApi', () => {
  ...
  it('handles timetable query, entries, validation, publishing with idempotency key', async () => {
    ...
    expect(urls[0]).toContain('/api/v3/timetables?semesterId=1&page=0&size=10')
    expect(urls[1]).toContain('/api/v3/timetables')
    expect(urls[2]).toContain('/api/v3/timetables/10')
    expect(urls[3]).toContain('/api/v3/timetables/10/entries?classId=5&session=MORNING')
    expect(urls[4]).toContain('/api/v3/timetables/10/entries')
    expect(urls[5]).toContain('/api/v3/timetables/10/validate')
    expect(urls[6]).toContain('/api/v3/timetables/10/review')
    expect(urls[7]).toContain('/api/v3/timetables/10/publish')
    expect(urls[8]).toContain('/api/v3/timetables/10/revisions')
    expect(urls[9]).toContain('/api/v3/timetable-calendars?semesterId=1')
    expect(urls[10]).toContain('/api/v3/timetables/my-timetable')
