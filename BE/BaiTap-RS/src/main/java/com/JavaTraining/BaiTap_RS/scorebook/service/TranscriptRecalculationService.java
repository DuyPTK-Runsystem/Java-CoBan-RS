...
public class TranscriptRecalculationService {
        ...
        private void calculateTerm(
                ...
                List<ClassSubject> classSubjects = classSubjectRepository
                                .findAllByClassIdAndSemesterIdOrderBySubjectIdAsc(enrollment.getCurrentClassId(),
                                                semester.getId())
                                .stream()
                ...
                termTranscript.setCalculationStatus(
                                currentSourceVersion == requestedVersion ? CalculationStatus.FINISH
                                                : CalculationStatus.IN_PROGRESS);
                if (currentSourceVersion == requestedVersion) {
                        termTranscript.setLastError(null);
                }
        ...
        private StudentSubjectTermResult calculateSubjectResult(
                ...
                StudentSubjectTermResult result = termResultRepository
                                ...
                                .orElseGet(() -> new StudentSubjectTermResult(
                                                termTranscriptId, classSubject.getId(), subject.getId(),
                ...
                if (scorebook != null) {
                        ...
                        if (subject.getSubjectType() == SubjectType.ACADEMIC) {
                                ...
                                SkillWeightConfig config = skillWeightRepository.findByScorebookId(scorebook.getId())
                                                .orElse(null);
                                result.setSkillScore(
                                                calculator.calculateSkillSubjectTermScore(config, columns, scores));
                        }
                }
                result.setCalculatedVersion(requestedVersion);
