...
class AcademicOfficeAttendanceServiceTest {
    ...
    private AcademicOfficeAttendanceService attendanceService;

    @BeforeEach
    void setUp() {
        attendanceService = new AcademicOfficeAttendanceService(
                sessionRepository,
                recordRepository,
                guard,
                auditService,
    ...
    @Test
    void upsertExceptionUpdatesExistingRecordAndWritesOfficeAudit() {
        AttendanceSession session = attendanceSession();
        ...
        attendanceService.upsertException(
                80L,
                2L,
                new ReqUpsertAttendanceExceptionDTO(AttendanceExceptionStatus.EARLY_LEAVE, "Giáo vụ điều chỉnh"));

        Mockito.verify(auditService)
                .writeRecordAudit("OFFICE_ATTENDANCE_EXCEPTION_UPDATED", session, beforeData, existing);
    }
    ...
    void upsertExceptionByCodeResolvesStudentAndReturnsMetadata() {
        ...
        ResAttendanceExceptionDTO response = attendanceService.upsertExceptionByCode(
                80L,
                "STU0000002",
                new ReqUpsertAttendanceExceptionDTO(AttendanceExceptionStatus.EXCUSED, "Giáo vụ xác nhận"));
    ...
    @Test
    void deleteExceptionDeletesRecordAndWritesOfficeAudit() {
        AttendanceSession session = attendanceSession();
